...
function TipPage() {
  ...
  useEffect(() => {
    const fetchRounds = async () => {
      try {
        const res = await axios.get("http://${import.meta.env.REACT_APP_API_BASE_URL}/matches/rounds");
    ...
    const fetchData = async () => {
      ...
      try {
        const matchesRes = await axios.get(
          `http://${import.meta.env.REACT_APP_API_BASE_URL}/matches?round=R${round}`
        );
        ...
        const userRes = await axios.get(
          `http://${import.meta.env.REACT_APP_API_BASE_URL}/users/${username}`
        );
  ...
  const submitTips = () => {
    const roundKey = `round${round}`;
    axios
      .post("http://${import.meta.env.REACT_APP_API_BASE_URL}/users/tip", {
        username,
        round: roundKey,
        tips,
      })
